...
<!-- @prompt 3-5条学习目标，每条≥25字，总字数≥80字。格式："能…（应用场景）"。具体能力描述而非概念罗列。必须达到80字以上（4条×20字或3条×27字）。 -->
{{learning_objectives}}

### 前置知识

<!-- @prompt 2-4个前置知识点，每条约15字以上，总字数≥30字。用[[wikilink]]关联。格式："- [[概念名]]: 为什么需要这个前置知识"。 -->
...
<!-- @prompt term_english: 标准英文术语，首字母大写。term_definition: 从源文摘抄标准定义，必须≥80字（不能低于80字）。用书名号引教材。如果源文不够长，结合自己的理解延伸补充。 -->
**{{name}}**（{{term_english}}）{{term_definition}}

### 2、精确释义

> <!-- @prompt 从源文精确获取完整的定义。含"是指"结构。≥50字。Markdown引文格式。 -->
> {{definition_sentence}}
> 来源：第{{source_chapter}}章 §{{source_from}} 

### 3. 分类与学科归属

<!-- @prompt domain: 学科领域。格式："大领域/子领域"。classification: 详细分类名称，必须≥30字。不能只写1个标签，要写完整的分类体系描述。 -->
- **学科领域**：{{domain}}
- **分类**：{{classification}}
...
<!-- @prompt 必须用$$...$$块级LaTeX包裹公式。从源文提取，每公式下方标注来源："*(来源：第N章 §X.X 式(N-XX))*"。如果该概念有关联的数学表达式，务必提取；如果确实没有公式，填"无"。不要因为偷懒填"无"——先搜索源文，确认无公式再填"无"。 -->
{{mathematical_model}}

### 3. 关键参数

<!-- @prompt ≥3个关键参数。格式："参数名(符号/单位): 说明"。每条≥10字。总字数≥30字。 -->
{{key_parameters}}
...
<!-- @prompt 2-3个应用场景，每个≥25字，总字数≥50字。格式："场景名: 背景→解决的问题→与该概念的关系"。 -->
