...
> P0 缺一不可，缺任一则教材感降级为科普笔记。P1 每章至少出现 2-3 项。P2/P3 按内容类型按需选择。
...
| 12 | **KB零结果时留空** | 即使零素材也要基于通用知识写出来，再报告 |
| 13 | **verify_chapter 的 summary 计数** | 脚本用 `^\d+\. \*\*` 匹配总结条目——但"2. 任何...**三要素**"内联粗体不匹配。已修复：改用 `^\d+\. `。如果自己写 check 脚本注意此正则 |
...
| `scripts/search_kb.py` | 知识库搜索（备选，通用文件搜索） |
| `scripts/detect_content_type.py` | 内容类型检测：6 种写作模式判别 |
| `scripts/gen_prompt.py` | **写作指令生成器**：综合大纲+KB+内容类型+6要素，输出每节提示词 |
...
| `references/six-elements.md` | 教材质量 6 要素检查指南 |
