...
| `migrate_yaml_schema.py --book-dir $DIR` | **v51.1** — YAML 数据版本迁移：自动读取 `dag_constants.REQUIRED_BD_FIELDS` 并逐字段比对。处理旧字段重命名（7组）、非法字段删除、必填字段补充。统一 `{items:[...]}` → 扁平列表。支持 `--dry-run`。 |\n| `enhance_solution_content(wiki_root, chapter)` | **v51.4** — 解答内容增强：检测通用模板文字→从题目关键词匹配源文相关段落→差异化生成。关键词映射从 `config/knowledge_keywords.yaml` 加载（领域无关，换书换文件）。 |\n| 外部配置 `config/book_info.yaml` | **v51.5** — 书籍总揽描述模板。`domain`、`book_template`、`chapter_descriptions` 三字段。`{book_name}`/`{domain}`/`{chapter_count}` 自动替换。`.dag/book_info.yaml` 可覆盖技能默认配置。 |\n| 外部配置 `config/knowledge_keywords.yaml` | **v51.5** — 领域知识关键词映射。`domain`+`keyword_maps` 字典（题目关键词→源文搜索词列表）。换领域只需改此文件。 |
...
| 81 | solutions.yaml 存在但 exercises.yaml 缺失 → 解答题目显示占位符、exercises 阶段跳过（0 文件） | **v51.6**: `build_kb_files.py` 在 solution 构建时检查 exercises.yaml 是否存在。若缺失 → 从 solutions.yaml 自动生成 exercises.yaml（提取 file/name/fm/bd.question）+ 触发 exercises build 子进程。注意 confidence 必须匹配 eval/exercise schema（0.65 非 0.75）。子进程路径用 os.path.abspath 避免 cwd 偏移。 |\n| 82 | 解答内容增强 `_detect_boilerplate` 使用固定正则模式("该习题考查教材第\\d+章")—Ch3 的"解答基于教材第3章..."就没匹配到，31个文件全部漏过 ❌ | **v51.7**: 改为**多维度内容质量评分**：(1) 长度 <60字 → too_short；(2) 模板词密度 >40%（该习题/基于/教材/核心/分析等15个高频模板词）→ high_boilerplate_density；(3) 不含任何领域术语且 <200字 → no_domain_terms。三条中任一条触发即替换。同时 `_extract_keywords` 增加章节标题回退：问题为占位符("第3章习题1")时从 `20_正文/第3章 电磁兼容预测.md` 提取章节名作为关键词。`_load_source_text` 跳过源文 YAML frontmatter。 |\\n| 83 | `_detect_boilerplate` 中 `domain_words` 硬编码了 40+ 个 EMC 关键词（麦克斯韦/法拉第/屏蔽/滤波/接地/EMC...）→ 换非EMC教材全部失效 🔥 | **v52.0**: `domain_words` 移除硬编码，改为从 `config/knowledge_keywords.yaml` 动态加载（key + 所有 mapped values 合并为领域术语集）。加载失败时跳过特异性检查，仅用长度+密度评分。 |\\n| 84 | `kb_graph_builder.py` 中 wikilink 路径规范化写死字符串 `"01_领域/01_资料库/"` 而非 `DIR` 常量 🔥 | **v52.0**: 改为 `f\"{DIR['DOMAIN_DIR']}/{DIR['LIBRARY_DIR']}/\"`。`quality_graph_checks.py` SQL 中 `LIKE '01_领域/%'` 同理改为 `f\"{DIR['DOMAIN_DIR']}/%\"`。 |\\n| 85 | 内容质量问题绝不能在输出端修补——YAML frontmatter 在 `_load_source_text` 阶段就应剥离，等进入 paragraph splitting 再擦已经晚了 🔥 | **核心原则**: 任何读取源文的函数（`_load_source_text` 等）必须在第一时间剥离 file2md 留下的 YAML frontmatter（`--- {title, source, parser, sha256} ---`）。`re.sub(r\"^---\\n.*?\\n---\\n\", ..., flags=re.DOTALL)`。脏数据一旦进入下游（paragraph splitting、keyword scoring）会不可逆地污染产出。\"先生成再擦除\"的思路是错的——应当在源头阻断。 |
