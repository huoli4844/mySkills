...
从提纲 docx 解析章节结构，创建 `output/写作大纲/writing-guide-chX.md` 骨架，包含空白板块：各教材章节对应关系、写作手法对比表、借鉴手法、盲区、结构建议表、每节写作指南。

同时输出 `output/outline_tasks.json`，供第二阶段使用。

**第二阶段：Agent 填充完整内容（关键环节）**

**大纲的厚度决定章节的厚度。** 同一份提纲、同一批参考书，大纲有详细每节指引时章节体量约100KB，无则约40KB（实战验证，差距2-3倍）。

Agent 读取 `output/outline_tasks.json`，对每个 `pending` 任务执行 `delegate_task` 填充以下全部板块：

| 板块 | 填充内容 | 作用 |
|:-----|:---------|:-----|
| 各教材章节对应关系 | 表格：每本书的章节内容 ↔ 我们的章节映射 | 确定素材来源 |
| 各教材写作手法对比表 | 9维度对比（核心手法/开篇/案例/术语/历史/深度/结构/可读性/读者） | 学习各书写法 |
| 各书最值得借鉴的3个手法 | 每本书3条+借鉴方式 | 明确可复用技法 |
| 各教材共同盲区 | 技术盲区（缺模型/缺对比表）+ 教学盲区（缺前沿/缺思政） | 确定发挥空间 |
| 结构建议表 | 每节体量(KB)、主导手法、素材来源 | 控制章节体量 |
| **每节写作指南** | **写作手法、必含要素(3-8条)、设问过渡(1-2句)、案例建议** | **核心：直接约束章节内容深度** |
| 素材清单/图表/衔接 | 按需填充 | 配套资源 |

⚠️ **不要只填结构建议表就完事。** 写作指南中的"必含要素"和"设问过渡"是 Agent 写章节时直接使用的素材——它们填充得越详细，章节体量越大、逻辑越流畅。
