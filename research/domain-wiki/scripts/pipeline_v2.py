...
def review_and_fix(book_dir: str, book_id: str, chapter: str,
    ...
    流程:
      1. 运行 quality_reviewer chapter --json 获取结构化审查结果
      2. 解析JSON: 识别分数低于threshold的文件和具体字段
      3. 输出FIX指令 (Agent读取后委托子Agent修复)
      4. 如果 --re-render: 重新渲染模板并重新审查

    Returns:
      True = 质量达标（无需修复）或修复后达标
      False = 需要修复
      (exit code 2 = 需要修复)
    """
    print("=" * 60)
    ...
    r = subprocess.run(
        [python, QUALITY_REVIEWER, "chapter",
         ...
         "--json", "--threshold", "0.01",
         "--fix-threshold", str(threshold)],  # 低exit阈值 + 高修复阈值
        capture_output=True, text=True
    )

    # 总是解析JSON检查fix_manifest（不论exit code）
    try:
    ...
    # Step 4: 如果 re_render，则重新渲染
    if re_render:
        print("\n▶ Step 4: 重新渲染（需Agent先完成修复）...")
        print("  ⏳ 请先运行: pipeline_v2.py review-fix --apply-fixes ...")
        print("  或手动委托子Agent修复YAML后重新渲染")
...
def apply_fixes_and_rerender(book_dir: str, book_id: str, chapter: str,
                              fix_manifest_json: str = ""):
    """Agent修复后：重新渲染 + 重新审查

    用法（在Agent完成YAML修复后）:
      python3 pipeline_v2.py review-fix --book-dir ... \\
        --book-id ... -c 3 --re-render --apply
    """
