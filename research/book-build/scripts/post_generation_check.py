...
检查项：
  ...
  3. Mermaid图闭合（每个```mermaid必须有对应的```）
  4. 常见拼写错误检查
  5. 自动修复：缺编号→补编号、编号跳跃→重新编号、重复→去重
...
def check_mermaid(text: str, verbose: bool = False) -> list:
    ...
    for i, block in enumerate(blocks[1:], 1):
        ...
        if end_marker < 0:
            issues.append((0, 'ERROR', f'Mermaid图{i}缺闭合```', True,
                          lambda t: t))  # Can't auto-fix easily
        else:
            # 检查Mermaid内容是否为空
            content = block[:end_marker].strip()
            if not content or len(content.split('\n')) < 2:
                issues.append((0, 'WARN', f'Mermaid图{i}内容过短', False, None))
